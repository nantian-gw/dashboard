...
export default function EditWasmPluginPage() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ["wasmplugin", "edit", namespace, name],
    queryFn: () => controlplane.get(`/v1/resources/wasmplugin/${namespace}/${name}`),
      ...
      <div className="flex justify-center py-8">
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            Failed to load WasmPlugin: {error instanceof Error ? error.message : "not found"}
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <WasmPluginForm
      ...
      onSuccess={() => {
        ...
        window.location.href = `/${locale}/wasm/plugins/${encodeURIComponent(name)}`;
      }}
    />
  );
}
