...
import {
  createEmptyUDPRouteFormData,
  udpRouteEditorCodec,
  udpRouteManifestToFormData,
...
interface UDPRouteFormProps {
  initialData?: UDPRouteFormData;
  mode: "create" | "edit";
  onSuccess?: () => void;
}

interface UDPRouteFormFieldsProps {
  value: UDPRouteFormData;
  onChange: (next: UDPRouteFormData) => void;
  disableIdentityFields: boolean;
}

function UDPRouteFormFields({
  value,
  onChange,
  disableIdentityFields,
}: UDPRouteFormFieldsProps) {
...
export function UDPRouteForm({ initialData, mode, onSuccess }: UDPRouteFormProps) {
  const t = useTranslations();
  const [formData, setFormData] = useState(initialData ?? createEmptyUDPRouteFormData());
  ...
  return (
    <ResourceEditorShell
      title={
        isEdit
          ? t("create.route.edit_title", { kind: "UDPRoute" })
          : t("create.route.title", { kind: "UDPRoute" })
      }
      description={
        isEdit
          ? t("create.route.edit_description", { kind: "UDPRoute" })
          : t("create.route.description", { kind: "UDPRoute" })
      }
      backHref="/routes"
      submitLabel={
        isEdit
          ? t("create.route.save", { kind: "UDPRoute" })
          : t("create.route.submit", { kind: "UDPRoute" })
      }
      submittingLabel={isEdit ? t("create.route.saving") : t("create.route.creating")}
      formData={formData}
      onFormDataChange={setFormData}
      codec={udpRouteEditorCodec}
      expectedEditIdentity={isEdit ? udpRouteEditorCodec.getIdentity(formData) : null}
      ...
      onSubmitManifest={async (yamlText) => {
        ...
        try {
          const nextFormData = udpRouteManifestToFormData(yamlText);
          ...
          const path = isEdit
            ? `/v1/resources/udproute/${formData.namespace}/${formData.name}`
            : "/v1/resources";
