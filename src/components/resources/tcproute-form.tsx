...
import {
  createEmptyTCPRouteFormData,
  tcpRouteEditorCodec,
  tcpRouteManifestToFormData,
} from "./tcproute-form-codec";
...
export interface TCPRouteFormData {
  name: string;
  namespace: string;
  gatewayName: string;
...
interface TCPRouteFormProps {
  initialData?: TCPRouteFormData;
  mode: "create" | "edit";
  onSuccess?: () => void;
}

interface TCPRouteFormFieldsProps {
  value: TCPRouteFormData;
  onChange: (next: TCPRouteFormData) => void;
  disableIdentityFields: boolean;
}

function TCPRouteFormFields({
  value,
  onChange,
  disableIdentityFields,
}: TCPRouteFormFieldsProps) {
  ...
  const addBackend = () => {
    onChange({
      ...value,
      backends: [...value.backends, { name: "", namespace: "default", port: 80, weight: 100 }],
    });
  };
            ...
            >
              ...
              <div className="grid w-20 gap-1">
                ...
                <Input
                  ...
                  value={backend.port}
                  onChange={(event) =>
                    updateBackend(index, "port", parseInt(event.target.value, 10) || 80)
                  }
                />
              </div>
              <div className="grid w-20 gap-1">
...
export function TCPRouteForm({ initialData, mode, onSuccess }: TCPRouteFormProps) {
  const t = useTranslations();
  const [formData, setFormData] = useState(initialData ?? createEmptyTCPRouteFormData());
  ...
  return (
    <ResourceEditorShell
      title={
        isEdit
          ? t("create.route.edit_title", { kind: "TCPRoute" })
          : t("create.route.title", { kind: "TCPRoute" })
      }
      description={
        isEdit
          ? t("create.route.edit_description", { kind: "TCPRoute" })
          : t("create.route.description", { kind: "TCPRoute" })
      }
      backHref="/routes"
      submitLabel={
        isEdit
          ? t("create.route.save", { kind: "TCPRoute" })
          : t("create.route.submit", { kind: "TCPRoute" })
      }
      submittingLabel={isEdit ? t("create.route.saving") : t("create.route.creating")}
      formData={formData}
      onFormDataChange={setFormData}
      codec={tcpRouteEditorCodec}
      expectedEditIdentity={isEdit ? tcpRouteEditorCodec.getIdentity(formData) : null}
      ...
      onSubmitManifest={async (yamlText) => {
        ...
        try {
          const nextFormData = tcpRouteManifestToFormData(yamlText);
          ...
          const path = isEdit
            ? `/v1/resources/tcproute/${formData.namespace}/${formData.name}`
            : "/v1/resources";
